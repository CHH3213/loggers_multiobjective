...
class DoubleEscape(core.Env):
    def __init__(self):
        ...
        self.target = np.array([2.5,0])
        # 初始距离
        self.distance_pv =0.8
        self.distance_barrier = 1.2
    ...
    def reset(self):
        ...
        # logger1.pose.position.x =   0.8 #+np.random.rand()*1.0-0.5#0.9
        # logger1.pose.position.y = +0.1 #+np.random.rand()*1.0-0.5
        logger1.pose.position.x =   1 #+np.random.rand()*1.0-0.5#0.9
        logger1.pose.position.y = -0.5 #+np.random.rand()*1.0-0.5
        ...
        logger2.pose.position.y = 0. #+np.random.rand()*2-1
                #
        ...
        # barrier.pose.position.x = 1.2  #1.5
        # barrier.pose.position.y = -0.2 #+np.random.rand()*2-1

        barrier.pose.position.x = 1.5  #1.5  实验1
        barrier.pose.position.y = -0.0 #+np.random.rand()*2-1
    ...
    def get_reward_done(self):
        ...
        # print('distance', self.distance_pv)
        if self.distance_pv <= 0.3:
            # r -=500
            done =True
    ...
    def take_e_action(self):
        ...
        delta_xy = np.array(self.obs[1][:2]) - np.array(self.obs[0][:2])
        # print(delta_xy)
        if delta_xy[1] > 0:
            if delta_xy[0] < 0:
                target2evader = np.arctan(
                    delta_xy[1]/delta_xy[0])  +np.pi# 目标点与evader之间的指向
            elif delta_xy[0] > 0:
                target2evader = np.arctan(
                    delta_xy[1]/delta_xy[0])   # 目标点与evader之间的指向
            else:
                target2evader = +np.pi/2
        elif delta_xy[1] < 0:
            if delta_xy[0] < 0:
                target2evader = np.arctan(
                    delta_xy[1]/delta_xy[0])+np.pi  # 目标点与evader之间的指向
    ...
    def take_barrier_action(self):
        ...
        if delta_xy[1]>=0:
                ...
                self.barrier2evader = 1/2*np.pi -0.001
        elif delta_xy[1]<0:
                self.barrier2evader = 1/2*np.pi  -0.001
        angle = self.barrier2evader

        if delta_xy[1] > 0 and delta_xy[0] < 0:
            angle = self.barrier2evader = 1/3*np.pi
        if delta_xy[1] < 0 and delta_xy[0] < 0:
            angle = self.barrier2evader = -1/3*np.pi
    ...
    def step(self, action_e):
        ...
        cmd_vel1 = Twist()
        # cmd_vel1.linear.y = self.ve * np.sin(action_e)*1.2 #test
        # cmd_vel1.linear.x = self.ve * np.cos(action_e)*0.9 #test
        ...
        cmd_vel2 = Twist()
        cmd_vel2.linear.x = self.v_barrier# (6,6)为0.58,方向为90
        for _ in range(1):
