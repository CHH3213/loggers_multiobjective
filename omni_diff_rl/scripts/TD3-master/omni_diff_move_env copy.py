...
class DoubleEscape(core.Env):
    def __init__(self):
        ...
        self.re_y = 0
        self.u_range = np.pi
        self.obs_dim = 9  # 每个agent的observation的维度,全乡+差分+移动障碍：3+3+3,障碍物的为：位置+速度
        ...
        # self.distance_goal =  np.sqrt(np.sum(np.square(np.array(self.obs[1][:2]) - np.array([6,6]) )))
    ...
    def calculate_state(self):
        ...
        if self.close_barrier == 1:
            k = ((yc-bs_y)/(-bs_x))
            # -----------------------------------------------------------------直行范围------------------------------------------------------#
            if ( abs(self.theta) < 1) and (abs(self.re_y) > yc):
    ...
    def reset(self):
        ...
        #barrier
        quart1 = euler_to_quaternion(0, 0, np.pi/2)
        ...
        barrier.pose.position.x = 6
        barrier.pose.position.y = 0
    ...
    def step(self, action_e):
        ...
        for _ in range(1):
            ...
            self.cmd_vel2_pub.publish(cmd_vel2)
        # self.flag=False
        self.rate.sleep()
