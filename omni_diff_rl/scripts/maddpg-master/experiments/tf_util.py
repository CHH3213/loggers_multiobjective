...
def save_state(fname, saver=None):
    ...
    # os.makedirs(os.path.dirname(fname), exist_ok=True) # python3
...
def function(inputs, outputs, updates=None, givens=None):
    ...
    if isinstance(outputs, list):
        # print('outpsjkh', outputs)
        return _Function(inputs, outputs, updates, givens=givens)
    elif isinstance(outputs, (dict, collections.OrderedDict)):
        # print('outpgfhjsjkh', outputs)
        f = _Function(inputs, outputs.values(), updates, givens=givens)
        return lambda *args, **kwargs: type(outputs)(zip(outputs.keys(), f(*args, **kwargs)))
    else:
        # print('outpukh', outputs)
        f = _Function(inputs, [outputs], updates, givens=givens)
        # print('sadnbgkjb==========', f)  # <tf_util._Function object at 0x7f013c02ea10>
